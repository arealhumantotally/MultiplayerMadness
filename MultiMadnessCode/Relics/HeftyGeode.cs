...
public class HeftyGeode() : MultiMadnessRelic
{
    ...
    {
        return PlatformUtil.GetPlayerName(RunManager.Instance.NetService.Platform, this.Owner.NetId);
    }
    public override bool TryModifyRestSiteOptions(Player player, ICollection<RestSiteOption> options)
    {
        if (_chipsRemaining == 0) return true;
        //Only adds Chip if another relic didnt already add it.
        if (options.Any((i) => i is Chip)) return true;
